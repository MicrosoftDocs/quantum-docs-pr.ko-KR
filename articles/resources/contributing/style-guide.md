...
식은 이며 `Sin(PI() / 2.0)`  `1.0` 프로그램이 나 그 이상 비트의 상태에 대해서는 아무 것도 의미 하지 않습니다.
...
- 진입점 작업의 이름을 "main"으로 지정 하지 마십시오.
- 진입점 작업의 이름을 일반 작업으로 지정 합니다.
...
작업 또는 함수가 직접 사용 하기 위한 것이 아니며, 부분 응용 프로그램에서 작동 하는 일치 하는 호출 가능에서 사용 해야 하는 경우에는 `internal` 부분적으로 적용 되는 호출 가능에 대해 키워드로 시작 하는 이름을 사용 하는 것이 좋습니다.
