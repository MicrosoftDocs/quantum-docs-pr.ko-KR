...
퀀텀 개발 키트에 대 한 최신 업데이트를 소개 합니다.  이 페이지에서는 새로운 Q # 언어 기능에 대 한 개요를 제공 하 고 기존 Q # 프로그램을 새 구문으로 [마이그레이션하](#Migration) 는 방법에 대해 설명 합니다.  [여기](xref:microsoft.quantum.guide.basics)에서 Q # 언어에 대 한 전체 설명은 전체 설명서를 참조 하세요.
...
조건식은 형식의 식입니다 `condition ? caseTrue | caseFalse` .
위의 예제에서 볼 수 있듯이 이제 배열 반복도 지원 됩니다.

및 내에서 할당 `using` `borrowing` 은 다음과 같은 단일의 비트, 이상 비트 배열 및 중첩 된 튜플에 대해 새로 될 수 있습니다.
```qsharp
    using (qubit = Qubit()) {
...
`invert` `distribute` `auto` 함수 특수화가 생성 되는 방법에 대 한 제어를 강화 하기 위해 두 개의 새로운 특수화 생성기 지시문 및가 추가 되었습니다.
작업 내에서 기본 특수화를 명시적으로 선언 하는 `body` 것은 더 이상 필수가 아닙니다.
...
한정자는 다른 식 조합 기 보다 엄격 하 게 바인딩되고, 특히 호출 식 조합 기 보다 더 엄격 `(` `)` 합니다. 
`Adjoint`, `Controlled` 및은 새로운 "래핑 해제" 후 위 연산자를 `!` 식 한정자로 처리 합니다. `Adjoint` `Controlled` 식 한정자로 및를 처리 하면 많은 사례에 대 한 괄호가 필요 하지 않습니다. 예를 들어, 다음 식은 `Op` 단일 작업, 단일 작업 `opArr` 배열 및 적절 한 인수를 전달 하는 데 사용할 수 있습니다 `arg` . 
```qsharp
    Adjoint Op (arg);
    Controlled opArr[i] (arg);
```
마이그레이션 스크립트는 대부분의 불필요 한 괄호를 삭제 하 고 새로운 기능을 최대한 활용 합니다. 그러나 스크립트는 코드를 해석 하지 않으며 드문 경우에 함수를 다르게 해석 하는 것을 고려 하지 않습니다 `Controlled` (부분 응용 프로그램과만 관련 됨).

New 한정자는 `!` 사용자 정의 형식을 기본 형식으로 캐스팅 합니다. 이 캐스트는 이전 릴리스에서 업 캐스트 고려 되었으며 자동으로 실행 되었습니다. 사용자 정의 형식에 대 한 새로운 해석을 사용 하면 더 이상 그렇지 않으며 캐스트를 명시적으로 만들어야 합니다.    
...
```
예제에서 볼 수 있듯이는 및와 `!` 같은 접두사 한정자 보다 엄격 하 게 `Controlled` 바인딩됩니다 `Adjoint` .

## <a name="breaking-changes"></a><a name="BreakingChanges"></a>주요 변경 내용 
...
아래에 나열 된 변경 내용 외에도 `__*__` *는 공백이 아닌 문자 시퀀스는 내부용으로 예약 되어 있으며 기호 이름으로 사용할 수 없습니다.  
...
이러한 변경에 대 한 한 가지 예는 사용자 정의 특수화의 기호 튜플입니다. 이전 릴리스에서는 제어 되거나 제어 된 adjoint 특수화 선언이 단일 기호 인수를 지정 했지만 0.3부터 시작 하 여, `...` 필요한 경우 컴파일러가 호출 가능한 선언에서 기호를 마이그레이션해야 함을 나타내는와 함께 모든 특수화에 대해 이러한 기호 튜플을 제공 하는 것이 좋습니다. 

## <a name="semantic-changes"></a>의미 체계 변경 

0.3 릴리스의 가장 중요 한 기능은 사용자 정의 형식이 형식 시스템에 통합 되는 방법입니다. 이전 릴리스에서는 사용자 정의 형식이 기본 형식의 하위 형식으로 간주 되었습니다. 이 릴리스와 앞으로 진행 되는 경우에는 더 이상 그렇지 않습니다. 이제 사용자 정의 형식은 고유 형식으로 간주 되며 사용자 정의 형식과 해당 기본 형식 간에 자동 캐스트가 없습니다. 모든 캐스트는 "래핑 해제" 연산자를 통해 명시적으로 만들어야 `!` 하며, 특히 사용자 정의 형식 개체의 내용에 액세스 하는 데 캐스트가 필요 합니다.  
...
다음 예에서는 함수를 사용 하 여 `ToOperation` 함수를 작업으로 변경 하는 방법을 보여 줍니다 `Square` `op` . 
```qsharp
...
## <a name="migrating-existing-projects-to-q-03"></a><a name="Migration"></a>기존 프로젝트를 Q # 0.3로 마이그레이션 
...
2. 텍스트 편집기에서 .csproj 파일을 편집 하 여 모든 "0.3.1811.2802"의 버전을 버전을 변경 합니다. `PackageReference` 예를 들면 다음과 같습니다.
```xml
...
```
4. 명령줄에서 명령줄 컴파일러에 통합 된 서식 도구를 사용 하 여 다음 명령을 실행 하 여 모든 구문 변경 내용을 처리 합니다.`dotnet msbuild /t:qsharpformat`  
    - 파일이 원위치에서 마이그레이션됩니다. 모든 원본 파일의 백업이 다음에 복사 됩니다.`obj\qsharp\.backup`
...
## <a name="migration-script"></a><a name="MigrationScript"></a>마이그레이션 스크립트 
...
- 이 프로젝트는 마이그레이션이 성공할 가능성이 있는지 확인 하기 위해 이전 버전의 퀀텀 개발 키트를 사용 하 여 빌드됩니다. 스크립트에 매개 변수를 사용 하 여이 단계를 건너뛸 수 있습니다 `-Force` .
- 새 프로젝트 템플릿이 설치 됩니다.
...
> Windows에서 실행 하는 등의 스크립트를 허용 하려면 `qdk-migrate.ps1` 실행 정책을 변경 해야 할 수 있습니다.
