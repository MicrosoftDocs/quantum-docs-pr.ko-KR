...
각 $N _e $ 파괴를 특정 상태 (또는 궤도)에 할당 하는 대신 두 번째 양자화는 각 궤도를 추적 하 고 각 궤도에 전자가 있는지와 동시에 전자 메일을 자동으로 확인 합니다. 해당 하는 웨이브 함수입니다.
...
두 번째 양자화의 예로, $ \psi_0\cdots \psi_{N-1} $은 공간 orbitals의 orthonormal 집합 이라고 가정 하겠습니다.
...
즉, 유효한 기본 상태는 $ \psi_{0, \uparrow}, \ldots, \psi_{N-1, \uparrow}, \psi_{0, \downarrow}, \ldots, \psi_{N-1, \downarrow} $ where $ \uparrow $ 및 $ \ldots $ 형식으로, 회전 각도의 두 eigenstates를 지정 하는 레이블입니다. 마음.
...
즉, wave 함수의 회전 및 공간 부분에 대 한 기본을 $ \psi_{0} \cdots \psi_{2N-1} $로 지정할 수 있습니다. 여기서 각 인덱스는 이제 $ (j, \시그마) $의 열거형입니다.
...
즉, $ \psi_1 $의 두 가지 법적 상태를 \begin{equation} \psi_1 \rightarrow \begin{cases} \ket{0}_1 & \text{if $ \psi_1 $가 사용 되지 않습니다.} \\\
\ket{1}_1 & \text{if $ \psi_1 $를 사용한 경우} \end{cases} \end{c}이 인코딩은 단일 퀀텀 비트로 전자 직업을 저장할 수 있으므로 퀀텀 컴퓨터에 유용 합니다.
...
이러한 설정은 각각 $a ^ \dagger_j $ 및 $a _j $로 표시 됩니다.

예를 들어, \begin{align} a ^ \dagger_1 \ket{0}_1 = \ket{1}_1, \quad a ^ \dagger_1 \ket{1}_1 = 0, \quad a_1 \ket{0}_1 = 0, \quad \ket \ket{1}_1 ={0}_1.
...
$K $ Fermions 시스템의 경우 두 번째 양자화 만들기 연산자 $a ^ \dagger_i $은 $ $ a ^ \dagger_i \ket{n_1, n_2, \ldots, 0_i, \ldots, n_k} = (-1) ^ {S_i} \ket{n_1, n_2, \ldots, 1_i에 의해 지정 됩니다. , \ldots, n_k}, $ $ 및 $ $ a ^ \dagger_i \ket{n_1, n_2, \ldots, 1_i, \lumnn_k} = 0, $ $ where $S _i = \sum_{j < i} a ^ \dagger_j a_j $는 단일 파티클의 상태 이며 인덱스 $j < i $ 인 총 Fermions 수를 측정 합니다.

세 번째 연산자는 두 번째 양자화 표현 에서도 종종 사용 됩니다.
이 연산자를 number 연산자 라고 하며 \begin{equation} n_i = a ^ \dagger_i a_i에 의해 정의 됩니다.
\end{equation}이 연산자는 지정 된 spin 궤도의 직업을 계산 합니다. 즉, \begin{align} n_i \ket{0}_i & = 0 \ nonumber\\을 말합니다 \
...
즉, $ $ a ^ \dagger_2 a ^ \dagger_1 \ket{0} =-a ^ \dagger_1 a ^ \dagger_2 \ket{0}입니다.
$ $ 이러한 연산자는 ' commute ' 이라고 $i 하며, j $는 \begin{align} a ^ \dagger_i a ^ \dagger_j =-(1-\delta_{i, j}) a ^ \dagger_j a ^ \dagger_i, \쿼드 a ^ \dagger_i a_j = \delta_{i, j}-a_j a ^ \dagger_i.
...
예를 들어 Hamiltonian $a _0 ^ \dagger a^ \dagger a_1 =-a_1 ^ \dagger a^ \dagger a_1 a_0 $를 예로 들어 보겠습니다.
...
\begin{equation} \hat{H} = \sum\_{pq} h\_{pq} a ^ \pqrs\_p a\_q + \frac{1}{2}\sum\_{pqrs} H\_{} a ^ \\_p a ^ \\_q a\_ra\_s + h\_{\textrm nuc}, \label{eq: totalHam} \end{s}. 여기서 $h\_{\textrm nuc} $는 핵 에너지 (Steiglitz 근사값에서 상수)입니다.

\begin{align} h\_{pq} & = \int\_{-\infty} ^ \infty \int ^\*\_p (x\_1) \int (-\frac{\nabla ^ 2}{2} + V (x\_1) \int) \int\_q (x\_1) \mathrm{d} ^ 3 x @no__ t_9_ 1, \end{align}

여기서 $V (x) $는 평균 현장 잠재력 이며

\begin{align} h\_{pqrs} & = \int\_{-\infty} ^ \infty \int\_{-\infty} ^ \infty\psi\_p ^\*(x\_1) \int\_q ^\*(x\_2) \ds\frac{1}{| x_1-x_2 |} \ right) \psi\_r (x\_2) \psi\_s (x\_1) \mathrm{d} ^ 3 x\_1 \ mathrm {d} ^ 3 x\_2. \ 레이블 {eq: 적분} \end{align}
...
예를 들어 $h _ {pq} a ^ \dagger_p a_q $는 spin 궤도 $q $에서 spin 궤도 $p $로의 전자 도약을 설명 합니다.
마찬가지로 $h _ {pqrs} a ^ \dagger_p a ^ \dagger_q a_r a_s $ (distinct $p, q, r, s $)에 대 한 두 개의 파괴는 spin orbitals $r $로, $s $ 분산은 서로 분리 되어 있으며 spin orbitals $p $ 및 $q $로 끝나도록 합니다.
$R = q $ 및 $p = s $ $h _ {prrp} a ^ \dagger_p a ^ \dagger_r a_r a_p = h_ {prrp} n_p n_r $는 두 파괴와 관련 된 에너지 페널티를 서로 가까이 제공 하지만 동적 프로세스를 설명 하지는 않습니다.
...
Hamiltonian $ \hat{H} = a_0 ^ \dagger a_1 + a_0 ^ \dagger $를 생각해 보세요.
```csharp
...
예를 들어 Hamiltonian $H = a_0 ^ \dagger a_1 ^ \dagger a_1 a_0-a_1 ^ \dagger a^ \dagger a_1 a_1 = 2a_0 ^ \dagger ^ \dagger $ (위에서 생성 된 용어의 합계)를 고려 합니다.
...
$ $ \begin{align} H = \sum\_{pq} H\_{pq} a ^ \sum\_{p} a\_{q} + \frac{1}{2}\sum\_{pqrs} H\_{} a ^ \sum {p} a ^ \sum\_{q} a\_t_10_ {r}\_{s}.
\end{align} $ $

이 표기법에는 최대 $N ^ 2 + N ^ 4 $ 계수가 있습니다.
그러나 이러한 계수 중 상당수는 동일한 연산자에 해당 하는 것으로 수집할 수 있습니다.
예를 들어 $p, q, r, s $가 고유 인덱스 인 경우 통신 규칙을 사용 하 여 $ $ a ^ \a\_{p} a ^ \aa\_{q} a\_{r} a\_{s} =-a ^ \aa\_{q} a ^ \aa\_{와 같이 표시할 수 있습니다. p}\_{r} a\_{s} =-a ^ \aas\_{p} a ^ \aa\_{q} a\_{s} a\_{r} = a ^ \aa\_{q} a ^ \aa\_{p} a\_{p} a\_{r}.
$$

뿐만 아니라 $H $가 Hermitian $h\_{pqrs} a ^ \dagger\_{p} a ^ \dagger} a\_{q} a\_{r} a\_{s} $와 같은 모든 비 Hermitian fermionic 연산자는 $H $에도 있는 켤레를 포함 합니다. 이러한 symmetries으로 분류 되는 용어 그룹의 인덱스를 고유 하 게 지정 하기 위해 $n + m $ fermionic 연산자 $a ^ \@no__t_4의 모든 시퀀스에서 $ (i\_1, \cdots, i\_n, j\_1, \cdots, j\_m) $ 인덱스에 대 한 정식 순서를 정의 _ {i\_1} \cdots a ^ \cdots {i\_n}\_{j\_1} \cdots a\_{j\_m} $as는 다음과 같습니다.
...
이 정식 순서를 사용 하는 경우 fermionic Hamiltonian는 $ $ \begin{align} H = \sum\_{(p, q) \sum S\_{1,1}} H '\_{pq} \frac{a ^ \sum\_{p} a\_{q} + a ^ \sum\_{q} a\_{ p}}{2}+ \sum\_{(p, q, r, s) \sum S\_{2,2}} h '\_{pqrs} \frac{a ^ \aa\_{p} a ^ \aa\_{q} a\_{r} a\_{s} + a ^ \ka\_{s} a ^ \\_{r} a\_{q} a\_{p}}{2}, \end{align} $ $은 적절 하 게 조정 된 1 ~ 2-전자 pq} $ 및 $h '\_{pqrs} $입니다.
