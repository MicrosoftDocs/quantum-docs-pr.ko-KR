...
3. 메뉴에서**솔루션 정리** **빌드** -> 를 선택 합니다.
4. 각 .csproj 파일에서 대상 프레임 워크를 (또는 `netcoreapp3.1` `netstandard2.1` 라이브러리 프로젝트인 경우)로 업데이트 합니다.
    즉, 다음 폼의 줄을 편집 합니다.
...
6. **도구** -> **Command Line**명령줄 -> **개발자 명령 프롬프트** 선택
...
3. 각 .csproj 파일에서 대상 프레임 워크를 (또는 `netcoreapp3.1` `netstandard2.1` 라이브러리 프로젝트인 경우)로 업데이트 합니다.
    즉, 다음 폼의 줄을 편집 합니다.
