...
Q#는 게이트 시퀀스를 해당 adjoint로 자동으로 컴파일하는 메서드를 제공 하 여이를 반영 합니다. 그러면 프로그래머가 많은 경우에 코드를 전달할 필요가 없습니다. 이에 대 한 예제는 다음과 같습니다.
...
이 예제는 간단한 예제 이지만 ( < f > 작업은 자체 adjoint), 보다 복잡 한 추가 비트 작업에 유용 하 게 사용할 수 있는 방법을 확인할 수 있습니다.
