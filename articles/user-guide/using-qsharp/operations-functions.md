...
작업은 Q #의 핵심입니다.
선언 되 면 기존 .NET 응용 프로그램에서 호출할 수 있습니다. 예를 들어 시뮬레이터를 사용 하거나 Q # 내의 다른 작업을 사용할 수 있습니다.
Q #에 정의 된 각 작업은 언어로 정의 된 기본 제공 기본 작업을 포함 하 여 원하는 수의 다른 작업을 호출할 수 있습니다. Q #에서 이러한 내장 작업을 정의 하는 특정 방법은 대상 컴퓨터에 따라 다릅니다.
...
퀀텀 작업의 adjoints는 퀀텀 컴퓨팅의 많은 측면에서 매우 중요 합니다. 유용한 Q # 프로그래밍 기법을 함께 설명 하는 이러한 상황에 대 한 예는이 문서의 [변화](#conjugations) 를 참조 하세요. 
...
다음 섹션에서는 Q # 코드에서 이러한 다양 한 특수화를 호출 하는 방법과 이러한 특수화를 지 원하는 작업을 정의 하는 방법을 설명 합니다.
...
> [!NOTE]
> Q #에서 제어 되는 버전은 항상 컨트롤의 배열을 사용 하 고, 제어는 항상 계산 ( `PauliZ` ) `One` 상태 ($ \ket $)에 있는 모든 컨트롤을 기반으로 합니다 {1} .
...
이 섹션에서는 Q # 작업 선언에 특수화의 존재를 포함 하 여 또는 함수와 함께 호출할 수 있는 기능을 제공 하는 방법에 대해 자세히 설명 합니다 `Adjoint` `Controlled` .
...
0.9 릴리스부터 Q #은 앞의 변환을 구현 하는 활용 문을 지원 합니다. 이 문을 사용 하 여 `ApplyWith` 다음과 같은 방법으로 작업을 구현할 수 있습니다.
...
Q #에서이를 구현 하려면 함수가 첫 번째 클래스 라는 사실을 활용 합니다.
...
Q #은 함수 및 작업에 *형식 매개 변수*를 포함 하 고 일반적인 튜플 매개 변수를 허용 하 여이 개념을 공식화 합니다.
